...
class MaterialsRequestsSection extends StatefulWidget {
  const MaterialsRequestsSection({Key? key}) : super(key: key);

  @override
  State<MaterialsRequestsSection> createState() =>
      _MaterialsRequestsSectionState();
}

class _MaterialsRequestsSectionState extends State<MaterialsRequestsSection> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels ==
      ...
      if (requestsCubit.state is GetAllRequestsSuccess &&
        ...
        requestsCubit.getAllRequests(isLoadMore: true, codeType: "MTRL");
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) =>
          sl.get<RequestsCubit>()..getAllRequests(codeType: "MTRL"),
      child: Builder(
        builder: (context) {
          ...
          return BlocBuilder<RequestsCubit, RequestsState>(
            builder: (context, state) {
              ...
              return Center(
                  child: Text("No payment requests available",
                      style: Styles.Rubic500(
                          fontSize: 20, color: ColorsManager.red)));
            },
          );
        },
      ),
